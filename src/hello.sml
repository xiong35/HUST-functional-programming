...
fun thermometer temp =
    if temp < 37 then "Cold"
    else
      if temp > 37 then "Warm"
      else "Normal"
...
fun evenly_positioned_elems [odd] = []  (* Base case: throw away *)
  | evenly_positioned_elems []    = []  (* Base case *)
  | evenly_positioned_elems (odd::even::xs) = even::evenly_positioned_elems xs
...
val err_msg = failing_function [1,2] 
  handle 
    Fail _ => "Fail was raised"
...
fun decrement_to_zero r = 
    if !r < 0 then (r := 0; r)
    else 
      (while !r >= 0 do r := !r - 1; r)
